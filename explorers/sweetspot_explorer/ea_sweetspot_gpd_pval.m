function [pval, tailFitUsed, apar, kpar, upar] = ea_sweetspot_gpd_pval(nullvals, empval, tail, Pthr)
...
% al. 2016, NeuroImage 141:502-516; method from Knijnenburg et al. 2009,
% Bioinformatics 25(12):i161-8; GPD fit from Hosking & Wallis 1987,
% Technometrics 29:339-349; goodness-of-fit test from Choulakian & Stephens
% 2001, Technometrics 43(4):478-484). Self-contained -- does not require
% the separate PALM toolbox on the MATLAB path.
%
% Why: the plain rank-based permutation p-value (see
% ea_sweetspot_nulldist_plot) has a resolution floor of 1/(Nperm+1) -- it
...
% matching how ea_sweetspot_nulldist_plot is used. It is deliberately not
...
%               (count+1)/(Nperm+1) convention as ea_sweetspot_nulldist_plot
...
switch tail
    ...
    case 'both'
        ...
        e = abs(empval);
    otherwise
        error('ea_sweetspot_gpd_pval:tail', 'tail must be ''right'', ''left'', or ''both''.');
end

% A permutation that produced no defined statistic (NaN) is evidence
% against the null, not missing data -- same convention as
% ea_sweetspot_nulldist_plot. It must never be counted as "exceeding" and
...
% Plain rank-based p-value first (same convention as ea_sweetspot_nulldist_plot).
if isnan(e) || nP == 0
    pval = NaN;
    tailFitUsed = false;
...
if rev
    [Gsorted, tailFrac] = ea_sweetspot_ecdf(g, 'descend');
else
    [Gsorted, tailFrac] = ea_sweetspot_ecdf(g, 'ascend');
end
...
while ~fitAccepted && q < nQ - 1
    ...
    A2pval = ea_sweetspot_andersondarling(ea_sweetspot_gpdpval(ytail, thisApar, thisKpar), thisKpar);
    ...
    if A2pval > 0.05
        ...
        if qualifies
            cte = numel(Gtail) / nP;
            pval = cte * ea_sweetspot_gpdpval(y, apar, kpar);
            tailFitUsed = true;
        end
...
% ==============================================================
function [Gsorted, tailFrac] = ea_sweetspot_ecdf(g, ord)
...
% ==============================================================
function p = ea_sweetspot_gpdpval(x, a, k)
...
% ==============================================================
function A2pval = ea_sweetspot_andersondarling(z, k)
