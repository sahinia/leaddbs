function tfcestat = ea_sweetspot_palm_tfce(X, datatype, geom, E, H, conn, deltah, area)
...
%   via palm_dpxlabel, ported alongside as ea_sweetspot_palm_dpxlabel.m) are
%   kept, even though only the volume branch has a caller in this codebase
%   today (ea_sweetspot_permtest_tfce.m) -- this is deliberate, so the
%   vertex/facewise path is already here, ported and untouched, whenever a
%   surface-based tool needs TFCE later.
...
%
% Usage:
%   tfcestat = ea_sweetspot_palm_tfce(X, datatype, geom, E, H, conn, deltah, area)
%
...
switch lower(datatype)
    ...
    case {'vertex','facewise'}
        ...
        if deltah == 0
            dh = maxX/100;
            for h = dh:dh:maxX
                dpxl = ea_sweetspot_palm_dpxlabel(D>=h, adj);
    ...
    otherwise
        error('ea_sweetspot_palm_tfce:datatype', 'datatype must be ''volume'', ''vertex'', or ''facewise''.');
end
